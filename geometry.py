class Geometry():
    ...
    def make_single_track(self, x_0, y_0, direction):
        ...
        endpts = {}
        for edge in self.get_edges():
            potential_endpt = endpts.append(edge.find_collision_point(x_0, y_0, direction))
            if potential_endpt != None:
                pot_x, pot_y = potential_endpt
                dist = edge.dist_to_collision(x_0, y_0, pot_x, pot_y)
                endpts[dist] = potential_endpt
        length, endpt = min(endpts)
        x_1, y_1 = endpt
        ...
        temp_y = y_0
        current_cell = self.which_cell(x, y, direction)
        while in_geometry(x, y, direction):
            temp_x += 0.0001*math.cos(math.rad(direction))
            temp_y += 0.0001*math.sin(math.rad(direction))
        ...
        for num in num_segments:
            current_cell = self.which_cell(x_i, y_i, direction)
        ...
        # don't forget to make Segment instances
 ##       ID = self.generate_ID('next_segment_ID')
        


        # add segments to the Track instance
